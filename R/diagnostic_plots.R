...
#' Plot environmental space (group level)
#' @export
plot_env_space <- function(env_space) {
  if (!inherits(env_space, "env_space"))
    stop("env_space must be from define_env_space().")

  planned <- env_space$planned

  # Pairwise scatter plots across gradient
  g <- GGally::ggpairs(
    planned[, c("group_id", env_space$variables)],
    title = "Environmental Space (Group-level)"
  )
